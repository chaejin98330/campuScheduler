...
def run(argv):
    ...
    for i in range(len(argv)):
        if argv[i] == "-l":                 #시간대 길이는 분 단위로 해주세요. 30분이면 30, 1시간이면 60, ...
            time_length = int(argv[i+1])
        elif argv[i] == "-n":               # -n 뒤에 개수 (요일 시작시간 끝시간) * 개수만큼 나옴
            time_number = int(argv[i+1])    # 0=월, 1=화, 2=수, 3=목, 4=금, 9.5=9시반, 16.5 = 4시반
            ...
            people_min = int(argv[i+1])
        elif argv[i] == "-P":
            people_max = int(argv[i+1])
        elif argv[i] == "-g":
            groupname = argv[i+1]
    ...
    #스케줄 클래스 만들기
    schedule = Schedule(equality_flag, duplication_flag, time_length, people_min, people_max, time_list)
